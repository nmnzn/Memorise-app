class MemosController < ApplicationController
  ...
  SYSTEM_PROMPT = <<~PROMPT
    ...
    3) Si l’utilisateur ne précise pas de nombre de questions, adapte la quantité selon le niveau demandé :
        - léger = 3 cards
        - moyen = 6 cards
        - profond = 10 cards
    4) Si le type de connaissances :
  ...
  def generate_cards_with_llm(prompt)
    chat = RubyLLM.chat
    chat.with_instructions(SYSTEM_PROMPT)

    # Retour brut du LLM selon le prompt du mémo
    response = chat.ask(prompt).content

    # Transformation du retour du LLM en tableau Ruby exploitable
    JSON.parse(response)
  end
end
